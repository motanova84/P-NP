...
def generate_margulis_gabber_galil_expander(n: int) -> nx.Graph:
    """
    Generate explicit expander graph using Margulis-Gabber-Galil construction.
    
    Args:
        n: Size parameter (graph will have n^2 vertices)
        
    Returns:
        Expander graph with good spectral properties
    """
    return nx.margulis_gabber_galil_graph(n)


def generate_tseitin_formula(G: nx.Graph, parity: bool = True) -> Tuple[int, List[List[int]]]:
    """
    Generate a Tseitin formula over a given graph.
    
    Args:
        G: Input graph
        parity: If True, assigns charges with odd total (unsatisfiable)
                If False, assigns charges with even total (satisfiable)
    ...
    # Create charge assignment based on parity
    if parity:
        # Odd total: make all vertices have charge 1 (unsatisfiable)
        charge_assignment = [1] * n
    else:
        # Even total: make all vertices have charge 0 (satisfiable)
        charge_assignment = [0] * n
...
if __name__ == "__main__":
    ...
    # Generate explicit expander
    print("Generating Margulis-Gabber-Galil expander...")
    G = generate_margulis_gabber_galil_expander(3)
    num_vars2, clauses2 = generate_tseitin_formula(G, parity=True)
    print(f"Generated formula with {num_vars2} variables and {len(clauses2)} clauses")
