...
def fase_emision(cantidad: int = 1995) -> dict:
    """Emite πCODE desde el reactor (fallback simbólico si colateral agotado)"""
    ts = datetime.now(timezone.utc).isoformat()
    try:
        from pi_code_reactor import reactor_emit_api
        resultado = reactor_emit_api(cantidad)
        if resultado.get("success"):
            log_cycle({"fase": "emision", "cantidad": cantidad, "resultado": resultado, "timestamp": ts})
            return resultado
    except: pass
    # Fallback: emisión simbólica (cuando colateral = 0)
    bloque = {"bloque_id": int(time.time()), "emision": cantidad,
              "coherencia": PSI, "frecuencia": F0,
              "timestamp": ts, "sello": SELLO, "modo": "SIMBOLICO", "tuyoyotu": True}
    bloque["hash"] = hashlib.sha3_512(json.dumps(bloque, sort_keys=True).encode()).hexdigest()
    log_cycle({"fase": "emision", "cantidad": cantidad, "hash": bloque["hash"][:16],
               "modo": "SIMBOLICO", "timestamp": ts})
    return {"success": True, "bloque": bloque, "modo": "SIMBOLICO"}
